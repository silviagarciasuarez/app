...
<style>
  ...
  @foreach ($editar_actividad as $item)
  <tbody>
    <tr>
...
<!-- 
            ...
            <td><form action="{{route('editar_viaje', $item->id)}}" method="post">
                        {{ csrf_field() }}
                       
                    <button type="submit"  style="cursor:pointer">Editar</button>
                    </form></td>  -->

                   <td><form action="{{route('eliminar_actividad', $item->id)}}" method="post">
                        {{ csrf_field() }}
